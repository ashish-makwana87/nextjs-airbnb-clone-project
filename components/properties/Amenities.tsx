 ...
 const noneSelected = amenitiesList.every((amenity) => amenity.selected === false)
  ...
  return (
    ...
    {amenitiesList.map((amenity) => {
     ...
     return <div key={amenity.name} className='flex items-center gap-x-3'><IoCheckboxOutline className='text-primary text-lg' /><p className=' capitalize'>{amenity.name}</p></div>
    })}
    </div>
    </div>
  )
